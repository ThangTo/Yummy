...
export default function CultureCardScreen() {
  ...
  const fetchCultureCard = useCallback(async () => {
    if (!params.foodId) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Cập nhật trạng thái global (user + passport) để Home / Explore / Profile tự phản chiếu dữ liệu mới
      ...
      Alert.alert('Đã sưu tầm', 'Món ăn đã được lưu vào hộ chiếu & mở khóa tỉnh tương ứng.', [
        {
          text: 'OK',
          onPress: () => {
            // Chỉ đóng screen hiện tại, tránh remount lại toàn bộ tab (map sẽ không phải load lại từ đầu)
            if (router.canGoBack()) {
              router.back();
            }
          },
  ...
  const handleRefresh = useCallback(async () => {
    try {
      setRefreshing(true);
      await Promise.all([fetchCultureCard(), refreshUser(), refreshPassport()]);
    } catch (err) {
      console.error('CultureCard refresh error:', err);
    } finally {
      setRefreshing(false);
    }
  }, [fetchCultureCard, refreshPassport, refreshUser]);
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.emptyStateContainer}>
          ...
          <TouchableOpacity style={styles.goHomeBtn} onPress={goBack}>
            <View style={[styles.goHomeGradient, { backgroundColor: colors.primary }]}>
...
const createStyles = (c: ThemeColors, mode: 'light' | 'dark') =>
  StyleSheet.create({
    ...
    cardContainer: {
      ...
      padding: 16,
      borderWidth: 1,
      borderColor: mode === 'light' ? '#e0d5d5' : '#2d1b1b',
    },
    heroImage: {
      width: '100%',
      height: 240,
    ...
    backButtonCircle: {
      ...
      height: 40,
      borderRadius: 20,
      backgroundColor: mode === 'light' ? '#f0f0f0' : 'rgba(255,255,255,0.1)',
      alignItems: 'center',
      justifyContent: 'center',
    },
    emptyStateContainer: {
